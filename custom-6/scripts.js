...
async function pollScanStatus() {
    ...
    try {
        ...
        // Silently handle transient connection errors during polling
    }
}
...
function renderPortAnalysis(allPorts) {
    ...
    allPorts.forEach(p => {
        const key = p.port;
        if (!portMap[key]) portMap[key] = { port: p.port, service: p.service, hosts: [] };
...
function renderSkippedHosts(skipped) {
    ...
    const count = document.getElementById('skippedCount');

    panel.style.display = 'block';
    count.textContent = skipped.length;
    list.innerHTML = '';

    skipped.forEach(host => {
        const item = document.createElement('div');
        item.className = 'skipped-item';
        item.innerHTML = `
            <div class="skipped-ip">${host.ip}</div>
            <div class="skipped-meta">
                ${host.hostname ? `<span class="skipped-hostname">${host.hostname}</span>` : ''}
                ${host.vendor ? `<span class="skipped-vendor">${host.vendor}</span>` : ''}
            </div>
            <div class="skipped-reason">${host.reason}</div>
            <div class="skipped-hint">Scan this host individually with a targeted IP (e.g., ${host.ip}/32)</div>
        `;
        list.appendChild(item);
    });
}
...
// ============================================================
// CSV EXPORT — includes scanned hosts + skipped hosts
// ============================================================
...
function exportResults() {
    ...
    const skipped = scanResults.skipped || [];

    // Sheet 1: Scan Results
    let csv = '=== SCAN RESULTS ===\n';
    csv += 'IP Address,Hostname,OS,Port,Protocol,Service,Version,Vulnerabilities\n';

    hosts.forEach(host => {
        if (host.ports && host.ports.length > 0) {
            host.ports.forEach(port => {
                const hostVulns = vulns
    ...
    // Sheet 2: Skipped Hosts
    if (skipped.length > 0) {
        csv += '\n=== SKIPPED HOSTS (timed out — scan individually) ===\n';
        csv += 'IP Address,Hostname,Vendor,Reason,Time Elapsed (seconds)\n';
        skipped.forEach(host => {
            csv += `"${host.ip}","${host.hostname || ''}","${host.vendor || ''}","${host.reason}",${host.elapsed || ''}\n`;
        });
    }

    // Sheet 3: All Vulnerabilities
    if (vulns.length > 0) {
        csv += '\n=== VULNERABILITY DETAILS ===\n';
        csv += 'Host,Port,Severity,Title,Description,Remediation\n';
        vulns.forEach(v => {
            csv += `"${v.host}","${v.port || ''}","${v.severity}","${v.title}","${v.description}","${v.remediation || ''}"\n`;
        });
    }
...
function resetResults() {
    ...
    document.getElementById('skippedPanel').style.display = 'none';
