...
const NEWS_PROXY = 'https://api.jonathan-castro.com/api/news/feed';
...
async function fetchStockQuote(symbols) {
    const results = [];
    // Use our own API proxy for Yahoo Finance (no more allorigins CORS issues)
    try {
        const resp = await fetch(`${API_BASE}/api/finance/quote?symbols=${symbols.map(s => encodeURIComponent(s)).join(',')}`);
        ...
        for (const sym of symbols) {
            try {
                const resp = await fetch(`${API_BASE}/api/finance/quote?symbols=${encodeURIComponent(sym)}`);
                if (!resp.ok) continue;
                const data = await resp.json();
                ...
                console.warn(`Failed to fetch ${sym}:`, e2);
            }
        }
    }
    return results;
...
async function loadFeaturedChart(symbol, name, range) {
    ...
    try {
        const resp = await fetch(`${API_BASE}/api/finance/chart?symbol=${encodeURIComponent(symbol)}&range=${params.range}&interval=${params.interval}`);
        if (!resp.ok) throw new Error(`API returned ${resp.status}`);
        const data = await resp.json();
...
async function loadMarketNews() {
    ...
    // Fetch market news from RSS feeds via our own API news proxy
    const marketFeeds = [
    ...
    for (const feedUrl of marketFeeds) {
        try {
            const resp = await fetch(`${NEWS_PROXY}?url=${encodeURIComponent(feedUrl)}`);
            if (resp.ok) {
                const data = await resp.json();
                if (data.items) {
            ...
            console.warn(`Market news feed failed for ${feedUrl}:`, e);
        }
    }

    // Fallback: try rss2json if our proxy returned nothing
