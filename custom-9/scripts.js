...
(function () {
    ...
    async function api(path, options = {}) {
        ...
        try {
            ...
            // For 4xx errors (like 409 Conflict), still parse the JSON so we get error messages
            if (!res.ok) {
                try {
    ...
    function renderStats(data) {
        ...
        setText('#statAttackerProfiles', data.unique_ips ?? 0);
        setText('#statCountries', data.countries ?? 0);
        setText('#statBlockedIPs', data.active_blocks ?? 0);
        // SSH attempts from attack_types array
        const sshEntry = (data.attack_types || []).find(t => t.event_type === 'ssh_brute_force');
        setText('#statSSHAttempts', sshEntry ? sshEntry.count : 0);
        setText('#statHoneypotEvents', data.honeypot_events ?? 0);
    }
    ...
    function renderHealth(data) {
        ...
        // health returns status: "ok" when working
        if (data.status === 'ok' && data.threat_events > 0) {
    ...
    function renderEventTypeChart(stats) {
        ...
        // attack_types is an array: [{event_type: "web_probe", count: 357}, ...]
        const typesArr = stats.attack_types || [];
        if (!typesArr.length) return;

        const labels = typesArr.map(t => eventLabel(t.event_type));
        const values = typesArr.map(t => t.count);
        const colors = typesArr.map(t => eventColor(t.event_type));
    ...
    function renderBlockedIPs(data) {
        ...
        if (Array.isArray(data)) {
            ...
            list = data.blocked;
        } else if (data && Array.isArray(data.ips)) {
            list = data.ips;
        } else if (data && Array.isArray(data.blocklist)) {
            list = data.blocklist;
        } else {
            list = [];
        }
    ...
    window._unblockIP = async function (ip) {
        ...
        // Backend uses DELETE /api/threat/block/:ip
        const result = await api('/api/threat/block/' + encodeURIComponent(ip), {
            method: 'DELETE',
        });
        ...
        if (result && (result.success || result.unblocked || result.status === 'ok')) {
            ...
            alert(result?.error || result?.message || 'Failed to unblock IP');
        }
    };
