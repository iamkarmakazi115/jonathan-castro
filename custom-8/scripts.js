/* ========================================================
   ...
   Fetches live RSS feeds through your own API proxy at
   api.jonathan-castro.com/api/news/feed
   ======================================================== */
...
// ─── RSS FETCHING (through your own API proxy) ────────────
async function fetchFeed(feedUrl) {
    try {
        const proxyUrl = `${API_BASE}/api/news/feed?url=${encodeURIComponent(feedUrl)}`;
        const resp = await fetch(proxyUrl);

        if (!resp.ok) {
            console.warn(`Feed proxy returned ${resp.status} for ${feedUrl}`);
            return [];
        }
        ...
        if (data.status === 'ok' && data.items && data.items.length > 0) {
            return data.items.map(item => ({
                ...
                source: item.source || extractDomain(feedUrl)
            }));
        }

        return [];
    } catch (e) {
        console.warn('Feed fetch error:', feedUrl, e.message);
        return [];
    }
}
...
async function loadCategory(catKey) {
    ...
    // Fetch all feeds in parallel across ALL sections for speed
    try {
        ...
        cat.sections.forEach((section, sIdx) => {
            section.feeds.forEach((feed, fIdx) => {
                allPromises.push(fetchFeed(feed.rss));
                promiseMap.push({ sectionIndex: sIdx, feedName: feed.name });
            });
        });
        ...
        cat.sections.forEach((section, sIdx) => {
            ...
            promiseMap.forEach((pm, pIdx) => {
                if (pm.sectionIndex === sIdx) {
                    const result = allResults[pIdx];
                    if (result.status === 'fulfilled' && result.value.length) {
                        articles = articles.concat(
                            result.value.map(a => ({ ...a, feedSource: pm.feedName }))
                        );
                    }
                }
            });
        ...
        // Cache results
        newsCache[catKey] = { sections: sectionResults, timestamp: Date.now() };

        loadingEl.classList.remove('visible');

        // Check if we got ANY articles at all
        const totalArticles = sectionResults.reduce((sum, s) => sum + s.articles.length, 0);
        if (totalArticles === 0) {
            errorEl.style.display = 'flex';
            errorEl.querySelector('p').textContent = 'Unable to reach the news proxy API. Make sure the backend news-proxy module is installed and the API service is running.';
        } else {
        ...
        section.articles.forEach((article, aIdx) => {
            ...
            card.innerHTML = `
                <div class="card-tag">
                    ...
                    ${escapeHTML(article.feedSource || article.source)}
                </div>
